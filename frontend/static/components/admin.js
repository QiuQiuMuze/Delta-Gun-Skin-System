const AdminPage = {
  render() {
    ...
    <div class="card" id="admin-mode-card">
      <h3>登录/注册模式</h3>
      <div class="input-row">
    ...
    <div class="card">
      ...
      <div id="list"></div>
      <div class="muted">标记：★ 表示快速注册（系统赠送 20000 法币），— 表示手机号注册的普通账号。</div>
    </div>
    ...
    <div class="card">
      <h3>删除账号（需验证码）</h3>
      <div class="muted">
        操作流程：1）输入要删除的用户名，点“获取删除验证码”；2）到后台查看验证码；<br>
    ...
    if (!modeToggle || !modeHint) {
      ...
      if (page) {
        ...
        card.innerHTML = `
          ...
          <div class="muted" id="admin-mode-hint">管理员模式开启后，注册/登录需要手机号与短信验证码。</div>
        `;
        const introCard = Array.from(page.children || []).find(el => el.classList && el.classList.contains("card"));
        if (introCard) {
          page.insertBefore(card, introCard.nextSibling);
        } else {
