...
def official_sell_layers(cfg: PoolConfig, state: BrickMarketState) -> List[Dict[str, Any]]:
    ...
    per_season_floor = 2000
    per_season_ceiling = 3200
    rng.shuffle(seasons)
    for idx, season in enumerate(seasons):
        qty = rng.randint(per_season_floor, per_season_ceiling)
        if qty <= 0:
            continue
...
def brick_purchase_plan(
    ...
    if remaining > 0:
        ...
        if remaining > 0:
            base_price = _sync_cfg_price(cfg, state)
            restock_season = season_key or BRICK_SEASON_FALLBACK
            restock_price = _season_brick_price(
                base_price,
                None if restock_season == BRICK_SEASON_FALLBACK else restock_season,
            )
            plan.append({
                "type": "official",
                "order": None,
